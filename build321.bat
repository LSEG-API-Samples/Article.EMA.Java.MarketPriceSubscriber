...
echo Building the ChainExpander application...
%JAVAC% -Xlint -d %BINDIR% src\com\thomsonreuters\platformservices\elektron\tools\chain\*.java
if %errorlevel% neq 0 goto :ERROR

echo Building the jar file...
%JAR% cfm %DISTDIR%\ChainExpander.jar Manifest.txt -C build\classes .
if %errorlevel% neq 0 goto :ERROR
...
:DONE
echo ** ChainExpander project successfully built **
