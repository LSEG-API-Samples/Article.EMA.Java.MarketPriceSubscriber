...
function build
{
   printf "Building the MarketPriceSubscriber application...\n"
   $JAVAC -Xlint -d $BINDIR src\com\thomsonreuters\platformservices\elektron\tools\marketprice\*.java; ret=$?
   if [ $ret != 0 ]; then
   ...
   $JAR cfm $DISTDIR/MarketPriceSubscriber.jar Manifest.txt -C build/classes .; ret=$?
   if [ $ret != 0 ]; then
      printf "Build failed.  Exiting\n"
      exit $ret
   fi
